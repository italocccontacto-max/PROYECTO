...
TARGET="blockade/src/main/java/com/irrovicas/blockade/data/local/mapper/PolicyEntityMapper.kt"
if [ ! -f "$TARGET" ]; then
  echo "ABORTA: no se encontró $TARGET (¿corriste setup_blockade_phase2_6.sh antes?)"
  exit 1
fi
echo "OK: $TARGET encontrado."

echo "== Reescribiendo PolicyEntityMapper.kt (fix: TargetEntity::toDomain / ConditionEntity::toDomain eran referencias a funciones member+extension a la vez, prohibidas por Kotlin -- se cambian a llamada lambda) =="
...
KOTLIN_EOF
echo "OK: PolicyEntityMapper.kt reescrito."
...
echo "============================================================"
echo "Fix aplicado: TargetEntity::toDomain y ConditionEntity::toDomain"
echo "eran referencias por método a funciones que, dentro de un object,"
echo "son miembro del object Y extensión del tipo a la vez -- Kotlin"
echo "prohíbe esa forma de referencia ahí (no es un bug de lógica, es"
echo "una restricción del compilador). Se cambiaron a { it.toDomain() },"
echo "que sí resuelve porque el object está implícito como receptor."
echo "No se tocó nada más del archivo."
echo ""
echo "Para commitear y subir:"
echo "  git commit -m \"fix(blockade): PolicyEntityMapper usa lambda en vez de referencia member+extension prohibida\""
